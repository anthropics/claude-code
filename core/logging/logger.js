...
class Logger {
  ...
  constructor(options = {}) {
    ...
    try {
      ...
      // Priority order: options > loggingConfig > DEFAULT_CONFIG.GLOBAL > DEFAULT_CONFIG
      this.config = {
        ...DEFAULT_CONFIG,
      ...
      // Ensure log level is set based on NODE_ENV if not explicitly configured
      if (!this.config.level && !loggingConfig.level) {
        this.config.level =
  ...
  log(level, message, metadata = {}) {
    ...
    const levelName =
      LEVEL_NAMES[level] ||
      Object.keys(this.config.customLevels).find(
        (name) => this.config.customLevels[name] === level
      ) ||
      "UNKNOWN";
    ...
    const formatter =
      this.formatters[this.config.format] || this.formatters.json;
  ...
  writeToConsole(level, formattedLog) {
    ...
    const outputStream =
      level >= LOG_LEVELS.ERROR ? console.error : console.log;
