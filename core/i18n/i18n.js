...
class I18n {
  ...
  loadConfig(options = {}) {
    ...
    // Get full config from config manager if available
    try {
      ...
      this.locale = options.locale || config.defaultLocale || DEFAULT_LOCALE;
      this.fallbackLocale =
        options.fallbackLocale || config.fallbackLocale || DEFAULT_LOCALE;
    } catch (err) {
      logger.warn("Failed to load i18n config", { error: err });
    }
  }
  ...
  setPluralRules(locale) {
    try {
      ...
      logger.warn(`Failed to set plural rules for locale ${locale}`, {
        error: err,
      });
  ...
  loadLocaleMessages(locale) {
    ...
    try {
      if (fs.existsSync(localePath)) {
        ...
        logger.debug(`Loaded locale messages for ${locale}`, {
          count: Object.keys(messages).length,
        });
      } else {
        logger.warn(`Locale file not found for ${locale}`, {
          path: localePath,
        });
      }
    } catch (err) {
      logger.error(`Failed to load locale messages for ${locale}`, {
        error: err,
      });
    }
  }
   ...
   */
  loadCustomMessages() {
    const customLocaleDir = path.join(
    ...
    try {
      if (fs.existsSync(customLocaleDir)) {
        ...
        for (const file of files) {
          if (file.endsWith(".json")) {
            ...
            try {
              ...
              logger.debug(`Loaded custom locale messages for ${locale}`, {
                count: Object.keys(messages).length,
              });
            } catch (err) {
              logger.error(
                `Failed to load custom locale messages for ${locale}`,
                { error: err }
              );
            }
          }
        ...
        logger.debug("No custom locale directory found", {
          path: customLocaleDir,
        });
      }
    } catch (err) {
  ...
  setLocale(locale) {
    if (!this.messages[locale]) {
      logger.warn(
        `Locale ${locale} not available, falling back to ${this.fallbackLocale}`
      );
      return false;
    }
  ...
  addMessages(messages, locale = null) {
    if (locale) {
      ...
      logger.debug(`Added messages for ${locale}`, {
        count: Object.keys(messages).length,
      });
    } else {
      ...
      for (const [loc, msgs] of Object.entries(messages)) {
        ...
        logger.debug(`Added messages for ${loc}`, {
          count: Object.keys(msgs).length,
        });
      }
    }
  }
  ...
  translate(key, params = {}, locale = null) {
    ...
    if (!messages || !messages[key]) {
      if (usedLocale !== this.fallbackLocale) {
        logger.debug(
          `Message ${key} not found in ${usedLocale}, falling back to ${this.fallbackLocale}`
        );
    ...
    if (!message) {
      if (this.debug) {
        logger.warn(
          `Message ${key} not found in ${usedLocale} or ${this.fallbackLocale}`
        );
        return `[${key}]`;
      }
      return key;
