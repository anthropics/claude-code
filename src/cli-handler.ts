...
import { getGlobalSeven, type SevenConfig } from './seven-wrapper';
import { getGlobalRegistry, type AgentName } from './routing/agent-router';
 ...
 */
export function parseAgentArgs(args: string[]): CLIOptions {
      ...
      case '-a':
        if (i + 1 < args.length) {
          ...
          if (isValidAgentName(agentName)) {
            options.agent = agentName as AgentName;
            i++;
          } else {
            throw new Error(
              `Invalid agent name: ${agentName}. Valid agents: explore, plan, execute, review`
            );
          }
        }
        break;
 ...
 */
export function showAgentStats(): void {
  const seven = getGlobalSeven();
 ...
 */
export function applyCLIOptions(options: CLIOptions): Partial<SevenConfig> {
  const config: Partial<SevenConfig> = {};
 ...
 * Main CLI handler
 */
export async function handleCLI(args: string[], userPrompt?: string): Promise<void> {
  ...
  const seven = getGlobalSeven(config);
 ...
 */
export function displayRoutingInfo(agent: AgentName, confidence: number, reasoning: string): void {
  const emoji = {
    explore: '🔍',
    plan: '📋',
    execute: '⚡',
 ...
 */
export function validateCLIConfig(options: CLIOptions): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
