...
const { SecurityReview } = require('./security_review');

// Import logger
const logger = require('../logging/logger').createLogger('security-check');
...
function printResultsSummary(results) {
  ...
  if (results.recommendations && results.recommendations.length > 0) {
    ...
    for (let i = 0; i < Math.min(3, sortedRecommendations.length); i++) {
      ...
      console.log(`${i + 1}. ${(rec.type === 'vulnerability' && (rec.severity === 'critical' || rec.severity === 'high')) 
        ? chalk.red(rec.title) 
