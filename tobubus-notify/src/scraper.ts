...
import type { BusApproach } from "./types.js";
...
export function parseApproaches(html: string): BusApproach[] {
  const $ = cheerio.load(html);
  const approaches: BusApproach[] = [];
  ...
  $("li > button.w-full").each((_, el) => {
    ...
    $el.find(".text-text-grey").each((_, grey) => {
      const t = $(grey).text().trim();
      if (t.includes("予測所要時間")) {
        estimatedTravelTime = t.replace("予測所要時間", "").trim();
      }
    });
...
function parseTimeRow(
  $: cheerio.CheerioAPI,
  $row: cheerio.Cheerio<Element>,
): BusApproach["departure"] {
