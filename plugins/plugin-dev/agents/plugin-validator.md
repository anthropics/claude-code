...
5. **Validate Agents** (if `agents/` exists):
   ...
   - For each agent file:
     ...
     - Or manually check:
       - Frontmatter uses only plugin-supported agent fields; required `name` and `description`, with supported optional execution and tool fields
       - Name format (lowercase, hyphens, 3-50 chars)
       - Description includes `<example>` blocks
       - Model is valid (inherit/sonnet/opus/haiku/fable or a full Claude model ID)
...
7. **Validate Hooks** (if `hooks/hooks.json` exists):
   ...
   - Or manually check:
     ...
     - Each hook group has a non-empty `hooks` array; `matcher` is optional and event-dependent
     - Hook type is valid for its event (`command`, `http`, `mcp_tool`, `prompt`, or `agent`)
