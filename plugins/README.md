...
| [pair-ai](./pair-ai/) | Debate-driven code development where two AIs collaborate - one implements, one critiques | **Command:** `/pair-code` - Start a pair programming session<br>**Agents:** `implementer` (writes code), `critic` (reviews for bugs, security, performance) |
