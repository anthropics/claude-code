...
| [new-session](./new-session/) | Start a fresh session from within an existing one, preserving the old session for `/resume` | **Command:** `/new` - Clean slate session while keeping old session resumable |
