...
| [handover](./handover/) | Export session context to a handover file for passing to another Claude session or LLM | **Command:** `/handover` - Exports conversation + git state as human-readable markdown; `/handover --llm` - Compact format optimized for LLM ingestion |
