...
| [preserve-session](./preserve-session/) | Preserves session history across project directory renames, moves, and copies by assigning each project a path-independent UUID | **Commands:** `/preserve-session:fix` - Recover sessions after rename/move, `/preserve-session:inherit` - Copy sessions from another project, `/preserve-session:doctor` - Diagnose session state, `/preserve-session:uninstall` - Permanently remove all preserve-session data<br>**Hook:** SessionStart - Auto-initializes project UUID on first run |
