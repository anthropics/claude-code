...
| [lean-agents](./lean-agents/) | Lightweight subagent replacements for users with many MCP servers whose built-in agents fail with "prompt too long" | **Agents:** `lean-explore`, `lean-plan`, `lean-general` (Sonnet with tool filtering), `lean-explore-safe`, `lean-plan-safe`, `lean-general-safe` (Opus with 1M context) |
