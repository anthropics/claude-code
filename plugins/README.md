...
| [test-writer](./test-writer/) | Write comprehensive, idiomatic tests for any codebase. Auto-detects the testing framework, fills coverage gaps, and validates tests run green | **Command:** `/test-writer` - 5-phase guided workflow: detect framework, analyze coverage, plan, write, and verify tests pass<br>**Agent:** `coverage-analyzer` - Maps exported symbols and branches, finds existing tests, returns prioritized gap list<br>**Skill:** `test-writer` - Auto-invoked when asked to write or add tests |
