...
| [code-quality-pipeline](./code-quality-pipeline/) | Quality gates between "code written" and "code merged" | **Skill:** `code-quality-pipeline` - A 4-step per-file review pipeline (Code Review → Simplification → Security → Final) plus a holistic pre-merge diff review |
