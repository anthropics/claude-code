...
- **Risks and open questions**: open risks, plus the single highest-value check you did not run (including for claims already scored 76 or above) and where you'd look first if this unit turns out to be wrong — when this coincides with a sub-76 claim's stated raise-path in Evidence, point to it rather than restating it. Also report incidental out-of-scope discoveries here (e.g. an adjacent pre-existing bug you noticed but correctly didn't touch), and if you saw a better decomposition than the one briefed, flag it here rather than acting on it.
