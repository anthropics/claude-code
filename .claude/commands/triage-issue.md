...
4. First, check if this issue is actually about Claude Code.
   - Look for Claude Code signals in the issue BODY: a `Claude Code Version` field or `claude --version` output, references to the `claude` CLI command, terminal sessions, the VS Code/JetBrains extensions, `CLAUDE.md` files, `.claude/` directories, MCP servers, Cowork, Remote Control, the Claude Desktop app (which embeds Claude Code), or the web UI at claude.ai/code. If ANY such signal is present, this IS a Claude Code issue — proceed to step 5.
   - Only if NO Claude Code signals are present: check whether a different Anthropic product (claude.ai chat, the standalone Claude Mobile app, the raw Anthropic API/SDK, or account billing with no CLI involvement) is the *subject* of the complaint, not merely mentioned for context. If so, apply `invalid` and stop. If ambiguous, proceed to step 5 WITHOUT applying `invalid`.
   - Note: The Claude Desktop app embeds Claude Code and is within scope for this tracker. Issues about the Desktop app's Claude Code integration, installation, or behavior should be triaged normally with the `area:desktop` label, not marked `invalid`.
