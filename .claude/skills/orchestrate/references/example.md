...
Applying the Load-bearing verification checklist: criterion 1 (dry-run performs zero deletes) is the single most load-bearing claim — the whole point of the feature — so it gets verified directly regardless of its 92 confidence: the orchestrator re-runs the pinned fixture-diff method itself and confirms byte-identical output. Criterion 2 is not load-bearing in the same way and scores 95, so a spot-check is optional (the orchestrator skims the diff anyway, since it's cheap). Criterion 3 scores 68 — below the threshold, and it is load-bearing (the log is the mechanism a real user would trust to know what dry-run would have done) — so the orchestrator verifies directly: runs the script against a fixture with zero matching files. The log call sits inside the same loop as the delete, so it correctly emits nothing — criterion 3 holds, and criterion 1's direct re-check also holds, so the orchestrator now has first-hand evidence on both instead of resting on the worker's confidence scores alone.
...
**Subjective or undefined-upfront criteria** ("make the onboarding feel polished," "improve the error messages"). If the decomposition itself depends on what "done" means, that's step 1's ambiguity gate — ask before decomposing. Otherwise, decompose normally, then before deploying use Criteria Ratification (in this file's companion, `references/patterns.md`): turn the subjective goal into the orchestrator's best concrete operationalization for each unit and echo it to the user, confirming or correcting before the team runs. Either way, guessing at a subjective target is the single most expensive place to guess wrong, since it invalidates every downstream unit.
