...
Environment Variables:
  ...
  console.log(`[DEBUG] Looking at issues older than #${maxIssueNumber}`);

  console.log(`[DEBUG] Fetching issues older than #${maxIssueNumber}...`);
  ...
  while (true) {
    ...
    // Filter to only include issues with numbers less than maxIssueNumber
    const filteredIssues = pageIssues.filter(issue => issue.number < maxIssueNumber);
    allIssues.push(...filteredIssues);
    
    // If we found an issue with a number less than our cutoff, we need to continue
    // but if the oldest issue in this page is still >= maxIssueNumber, we can stop
    ...
    if (oldestIssueInPage && oldestIssueInPage.number >= maxIssueNumber) {
      ...
      console.log(`[DEBUG] No issues in page #${page} are older than #${maxIssueNumber}, stopping`);
      break;
    }
  ...
  console.log(`[DEBUG] Found ${allIssues.length} issues older than #${maxIssueNumber}`);
