...
| File | Purpose |
|------|---------|
| `versions.tf` | Provider pins (google, random, tls) |
| `variables.tf` | All inputs (defaults match `setup.sh`'s) |
| `main.tf` | Resources |
| `lb.tf` | Optional internal Application Load Balancer (off by default — see below) |
...
`INGRESS_TRAFFIC_INTERNAL_LOAD_BALANCER` and front the service with an internal
Application Load Balancer — either your own, or the optional one this module can
provision (below).

## Internal ALB (optional)

`lb.tf` can provision the internal Application Load Balancer for you:
a proxy-only subnet, a serverless NEG on the Cloud Run service, an
`INTERNAL_MANAGED` backend service / URL map / HTTPS proxy, a static internal
IP forwarding rule on 443, and a VPC-private Cloud DNS zone resolving your
chosen hostname to the LB. TLS uses a self-signed certificate generated by the
`tls` provider (key and cert live in state); clients trust it explicitly with
`curl --cacert` / `NODE_EXTRA_CA_CERTS` using the `lb_ca_cert_pem` output —
swap in a CA-issued regional certificate for anything longer-lived.

```hcl
ingress              = "INGRESS_TRAFFIC_INTERNAL_LOAD_BALANCER"
internal_alb_enabled = true
lb_hostname          = "claude-gateway.gw.example.com"
```

Then set gateway.yaml `public_url` to `https://<lb_hostname>`, add the
proxy-only subnet CIDR (`proxy_only_subnet_range`, default `10.0.2.0/24`) to
`listen.trusted_proxies`, and register `https://<lb_hostname>/oauth/callback`
on the OAuth client. The hostname resolves only inside the VPC (private zone),
so pick any domain you control conventions for; the LB IP is in the
`lb_ip` output.

Note the ALB changes network reachability only — the Cloud Run **invoker IAM
check still applies** to traffic arriving through it, so the
`invoker_iam_disabled` / `allow_unauthenticated` choice above is still
required either way.
