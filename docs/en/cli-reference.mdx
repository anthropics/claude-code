...
| `--agents`                             | Define custom [subagents](/en/sub-agents) dynamically via JSON (see below for format)                                                                                                                     | **Bash/PowerShell:** `claude --agents '{"reviewer":{"description":"Reviews code","prompt":"You are a code reviewer"}}'`<br/><br/>**Windows CMD:** `claude --agents "{\"reviewer\":{\"description\":\"Reviews code\",\"prompt\":\"You are a code reviewer\"}}"` |
...
| `--json-schema`                        | Get validated JSON output matching a JSON Schema.<br/><br/>**Bash/PowerShell:**<br/>`claude -p --json-schema '{"type":"object"}' ...`<br/><br/>**Windows CMD:** Use double quotes and escape internals:<br/>`claude -p --json-schema "{\"type\":\"object\"}" ...` | **Bash/PowerShell:** `claude -p --json-schema '{"type":"object","properties":{...}}' "query"`<br/><br/>**Windows CMD:** `claude -p --json-schema "{\"type\":\"object\",\"properties\":{...}}" "query"` |
...
> **Windows CMD Note:** Windows Command Prompt does not support single-quoted strings or multi-line commands in the same way as Bash/PowerShell. To use this flag in CMD, you must provide the JSON as a single line and escape all internal double quotes (e.g., `{\"key\":\"value\"}`).
