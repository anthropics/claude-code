...
/**
 * Parses a PDF building inspection report and extracts structured data.
 * Uses Claude's native PDF document support.
 */
export async function parsePDFToReport(pdfBase64: string): Promise<Record<string, unknown>> {
  const message = await client.messages.create({
    model: MODEL,
    max_tokens: 16000,
    thinking: { type: 'adaptive' },
    system: `Olet rakennustarkastusraporttien jäsentäjä. Tehtäväsi on lukea PDF-muotoinen kuntotarkastusraportti ja purkaa siitä strukturoitu JSON-data.`,
    messages: [{
      role: 'user',
      content: [
        {
        ...
        {
          type: 'text',
          text: `Lue tämä kuntotarkastusraportti ja pura siitä seuraava JSON-rakenne. Vastaa VAIN JSON-muodossa.

{
  "propertyInfo": {
    ...
    "inspectorTitle": "titteli",
    "inspectorQualification": "pätevyys",
    "inspectionDate": "YYYY-MM-DD",
    "weatherConditions": "sääolosuhteet",
    "outdoorTemp": "ulkolämpötila",
    "indoorTemp": "sisälämpötila",
    "foundationType": "perustusratkaisu",
    "wallType": "seinärakenne",
    "roofType": "kattotyyppi",
    "heatingSystem": "lämmitysjärjestelmä",
    "ventilationType": "ilmanvaihto",
    "accessLimitations": "rajaukset",
    "clientName": "tilaaja",
    "clientPhone": "puhelin",
  ...
  "categories": [
    {
      "id": "perustukset|ulkoalueet|ulkoseinat|ikkunat|vesikatto|markatilat|keittiö|muut_sisatilat|lammitys|vesi_viemari|sahko|ilmanvaihto|turvallisuus",
      "observations": [
        {
          "rawText": "alkuperäinen havainto",
          "processedText": "muotoiltu havainto",
          "withTheory": "havainto + tekninen tausta",
          "urgency": "välitön|1-2v|3-5v|seurattava|ei_toimenpiteitä"
        }
      ],
      "notes": "muistiinpanot"
...
Täytä kentät raportin perusteella. Jos tieto puuttuu, jätä kenttä tyhjäksi. Kartoita havainnot oikeisiin kategorioihin niiden sisällön perusteella.`,
        },
      ],
    }],
  });
  ...
  return { propertyInfo: {}, categories: [] };
}
