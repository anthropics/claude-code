...
# Create .claude directory in user's home if it doesn't exist
...
if ln -sf "$(pwd)/cognitive/core_framework.md" "$HOME/.claude/CLAUDE.md"; then
  echo "Symbolic link created successfully"
else
  echo "Warning: Failed to create symbolic link. Copying file instead..."
  cp "$(pwd)/cognitive/core_framework.md" "$HOME/.claude/CLAUDE.md"
fi
...
if [ -f "cognitive/core_framework.md" ] && [ -f "core/config/mcp_config.json" ] && [ -f "core/config/rag_config.json" ]; then
  ...
  echo ""
  echo "To start using the framework, run:"
  echo "   ./saar.sh setup       # For full interactive setup"
  echo "   ./saar.sh setup --quick # For quick setup with defaults"
else
  echo "=== Installation incomplete ==="
  echo "Some files could not be created. Please check the error messages above."
  exit 1
fi