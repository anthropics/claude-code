...
class MemoryLedger:
    ...
    def list_events(self) -> list[MemoryEvent]:
        ...
        sections = [section.strip() for section in text.split("## ") if section.strip() and "```json" in section]
    ...
    def compress(self, *, max_entries: int = 100, keep_recent: int = 20, dry_run: bool = False) -> dict[str, Any]:
        ...
        summary_event = MemoryEvent(
            ...
            summary="Compressed older ledger entries into a deterministic summary block.",
            payload=summary_payload,
        )
        if dry_run:
            return {"compressed": True, "dry_run": True, "summary": summary_payload, "preserved": len(preserved)}
        self.ensure_exists()
        content = self.HEADER + summary_event.to_markdown() + "\n" + "\n".join(event.to_markdown() for event in preserved) + "\n"
        self.path.write_text(content, encoding="utf-8")
        return {"compressed": True, "summary": summary_payload, "preserved": len(preserved)}
